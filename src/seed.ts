...
export async function seedDatabase(forceReset: boolean = false) {
  try {
    if (!AppDataSource.isInitialized) {
      ...
      console.log('🔌 Conexión de sembrado iniciada independientemente.');
    }
    ...
    if (count > 0 && !forceReset) {
      console.log('ℹ️ La base de datos ya tiene información.');
      return; 
    }
    ...
    if (forceReset || count === 0) {
      ...
      console.log('📦 Insertando estados y municipios...');
